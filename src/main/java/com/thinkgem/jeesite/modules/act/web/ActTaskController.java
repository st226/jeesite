...
public class ActTaskController extends BaseController {
	...
	//	List<Act> list = actTaskService.todoList(act);
		...
		List<WFWorkItem> list = bps.getUserTasks4SDO(UserUtils.getUser().getId(), "ALL", "ALL") ;
		model.addAttribute("list", list);
		for (WFWorkItem wfWorkItem : list) {
			wfWorkItem.setStartTime(wfWorkItem.getStartTime().substring(0, 4)+"-"+wfWorkItem.getStartTime().substring(4, 6)+"-"+wfWorkItem.getStartTime().substring(6, 8));
		}
	 ...
	 @RequestMapping(value = "getSelectAct")
	    @ResponseBody
	    public void getSelectColumnData(String busTableType,HttpServletResponse response){
		    BpsManage bps = new BpsManage() ;
			List<WFWorkItem> list = null ;
			try {
				list = bps.getUserTasks4SDO(UserUtils.getUser().getId(), "ALL", "ALL");
