...
<body>
	...
	<form:form id="inputForm" modelAttribute="testDataMain" action="${ctx}/test/testDataMain/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="name" htmlEscape="false" maxlength="100" cssClass="required" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="sex" class="input-xlarge ">
					...
					<form:options items="${fns:getDictList('sex')}" itemLabel="label" cssClass="required" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
				...
				<div class="controls">
					...
					<script type="text/template" id="testDataChildTpl">//<!--
						<tr id="testDataChildList{{idx}}">
							...
							<td>
								<input id="testDataChildList{{idx}}_name"   cssClass="required"  name="testDataChildList[{{idx}}].name" type="text" value="{{row.name}}" maxlength="100" class="input-small "/>
							</td>
							
<td>


<sys:treeselect id="testDataChildList{{idx}}_area" name="testDataChildList[{{idx}}].remarks" value="${row.remarks}" 
				labelName="row.remarks" labelValue="${row.remarks}"
