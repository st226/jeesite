...
<body>
	...
	<form:form id="inputForm" modelAttribute="swOrder" action="${ctx}/business/order/swOrder/save" method="post" class="form-horizontal">
		...
		<legend>采购设备明细</legend>
					<table id="contentTable" class="table table-striped table-bordered table-condensed">
						...
						<tbody id="swProductList">
						</tbody>
						<shiro:hasPermission name="business:agreement:swAgreement:edit"><tfoot>
							<tr><td colspan="9"><a href="javascript:" onclick="addRow('#swProductList', swAgreementPayRowIdx, swProductTpl);swAgreementPayRowIdx = swAgreementPayRowIdx + 1;" class="btn">新增</a></td></tr>
						</tfoot></shiro:hasPermission>
					</table>
					<script type="text/template" id="swProductTpl">//<!--
						<tr id="swProductList{{idx}}">
							<td class="hide">
								<input id="swProductList{{idx}}_id" name="swProductList[{{idx}}].id" type="hidden" value="{{row.id}}"/>
								<input id="swProductList{{idx}}_delFlag" name="swProductList[{{idx}}].delFlag" type="hidden" value="0"/>
							</td>
							<td>
								<input id="swProductList{{idx}}_productName" name="swProductList[{{idx}}].productName" type="text" value="{{row.productName}}"  class="input-small "/>
							</td>
							<td>
								<input id="swProductList{{idx}}_productType" name="swProductList[{{idx}}].productType" type="text" value="{{row.productType}}" class="input-small "/>
							</td>
							
							<td>
								<input id="swProductList{{idx}}_productMade" name="swProductList[{{idx}}].productMade" type="text" value="{{row.productMade}}" class="input-small "/>
							</td>
							
							<td>
								<input id="swProductList{{idx}}_unitPrice" name="swProductList[{{idx}}].unitPrice" type="text" value="{{row.unitPrice}}" maxlength="255" class="input-small "/>
							</td>
							<td>
								<input id="swProductList{{idx}}_productAmount" name="swProductList[{{idx}}].productAmount" type="text" value="{{row.productAmount}}" maxlength="255" class="input-small "/>
							</td>
                             <td>
								<input id="swProductList{{idx}}_totalPrice" name="swProductList[{{idx}}].totalPrice" type="text" value="{{row.totalPrice}}" maxlength="255" class="input-small "/>
							</td>
                            <td>
								<input id="swProductList{{idx}}_officeName" name="swProductList[{{idx}}].officeName" type="text" value="{{row.officeName}}" maxlength="255" class="input-small "/>
							</td>
                            <td>
								<input id="swProductList{{idx}}_userName" name="swProductList[{{idx}}].userName" type="text" value="{{row.userName}}" maxlength="255" class="input-small "/>
							</td>
                             <td>
								<input id="swProductList{{idx}}_productUse" name="swProductList[{{idx}}].productUse" type="text" value="{{row.productUse}}" maxlength="255" class="input-small "/>
							</td>
							<shiro:hasPermission name="business:agreement:swAgreement:edit"><td class="text-center" width="10">
								{{#delBtn}}<span class="close" onclick="delRow(this, '#swProductList{{idx}}')" title="删除">&times;</span>{{/delBtn}}
					...
					<script type="text/javascript">
						...
						$(document).ready(function() {
							var data = ${fns:toJson(swOrder.swProductList)};
							for (var i=0; i<data.length; i++){
								addRow('#swProductList', swAgreementPayRowIdx, swProductTpl, data[i]);
	...
	<iframe src="${ctx}/business/order/swOrder/progress?id=${swOrder.id}" style="width: 100%;height: 3000px"  marginwidth="0" marginheight="0" frameborder="0"></iframe>
</body>
</html>