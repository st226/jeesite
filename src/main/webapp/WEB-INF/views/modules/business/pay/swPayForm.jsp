...
<head>
	...
	<script type="text/javascript">
		...
		function tt(id){
			if(id=="1"){
				window.open("${ctx}/business/pay/swPay/view?id=${swPay.id}",'top');
			}else{
				window.open("${ctx}/business/pay/swPay/view2?id=${swPay.id}",'top');
			}
...
<body>
	...
	<form:form id="inputForm" modelAttribute="swPay" action="${ctx}/business/pay/swPay/save" method="post" class="form-horizontal">
		...
		<table class="table-form">
				...
				<tr>
				     <td class="tit" width="15%">经办人：</td><td width="35%">
							<form:input path="field1" htmlEscape="false" maxlength="255" class="input-xlarge required"/>
				     <span class="help-inline"><font color="red">*</font> </span>
					</td>
		...
		<legend >本次付款</legend>
			<table  class="table table-striped table-bordered " >
				...
				<th >联系电话</th>
						...
						<c:forEach items="${payListwf}" var="pay"  varStatus="vs">
			        <tr>
			        <td>
				   <input type="checkbox" id="field4${vs.index}" name="checkbox_name[${vs.index}]" onclick="checkOk('${pay.id}','${vs.index}')" value="1" ${pay.field4 eq '1' ? 'checked' : ''}/>
				    <input type="hidden" name="columnList[${vs.index}].id" value="${pay.id}"/>
				      <input type="hidden" name="columnList[${vs.index}].contrateTreat" value="${pay.contrateTreat}"/>
				</td>
			        <td  >${vs.index+1}</td>
		...
		<div class="form-actions">
			<shiro:hasPermission name="business:pay:swPay:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="暂 存"/>&nbsp;</shiro:hasPermission>
			<input id="btnSubmitok" class="btn  btn-primary"   type="button"   onclick="ok(1)"  value="提交"/>
			<input id="dayin" class="btn" type="button"  onclick="tt(1)" value="请款单预览"/>
			<input id="dayin" class="btn" type="button"  onclick="tt(2)" value="借款单预览"/>
