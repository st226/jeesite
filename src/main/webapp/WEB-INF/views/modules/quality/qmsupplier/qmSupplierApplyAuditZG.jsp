...
<head>
	...
	<script type="text/javascript">
		...
		function ok(e){
			alert($("#field3Id").val());
			alert($("#field5Id").val());
			if( $("#field3Id").val()==null || $("#field3Id").val()=="" || $("#field5Id").val()==null ||$("#field5Id").val()==""){
				alert("请先选择审批人");
			}else{
				$("#state").val("1");
			    $("#inputForm").submit();
		...
		function test(pid){
			top.$.jBox.open("iframe:http://10.193.200.16:8088/default/archives/archives/imageFlow.jsp?id=${qmSupplier.processinstid}", 
		...
		function orderType(e){
			type=e.value;
			if(type=='1'){
				document.getElementById("field8Tr").style.display="" ;
			}else{
				document.getElementById("field8Tr").style.display="none" ;
			}
		}
...
<body>
	...
	<form:form id="inputForm" modelAttribute="qmSupplier" action="${ctx}/quality/qmsupplier/qmSupplier/submit" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<center><legend >新增供应商申请表</legend></center> 
		<table class="table-form">
				...
				<tr>
					...
					<td class="tit" width="15%">产品名称：</td><td width="35%" colspan="1">
						<form:input path="product" htmlEscape="false" maxlength="100" class="input-xlarge"/>
	
					</td>
					<td class="tit" width="15%">供应商类型：</td><td width="35%" colspan="1">
				...
				<tr>
					...
					<tr>
					<td class="tit" width="15%">供应商准入评定申请表：</td><td width="35%" colspan="3">
							<form:hidden id="field5" path="field5" htmlEscape="false" maxlength="2000" class="input-xlarge"/>
				<sys:ckfinder input="field5" type="files" uploadPath="/quality/qmsupplier/qmSupplier" selectMultiple="true"/>
				
					</td>
					
				</tr>
					<tr>
					<td class="tit" width="15%">供应商准入评定自评报告：</td><td width="35%" colspan="3">
							<form:hidden id="field6" path="field6" htmlEscape="false" maxlength="2000" class="input-xlarge"/>
				<sys:ckfinder input="field6" type="files" uploadPath="/quality/qmsupplier/qmSupplier" selectMultiple="true"/>
						...
						<tr id="qmSupplierAdmittanceList{{idx}}">
							<td class="hide">
								<input id="qmSupplierAdmittanceList{{idx}}_id" name="qmSupplierAdmittanceList[{{idx}}].id" type="hidden" value="{{row.id}}"/>
                                 <input id="qmSupplierAdmittanceList{{idx}}_remarks" name="qmSupplierAdmittanceList[{{idx}}].remarks" type="hidden" value="{{row.remarks}}"/>
		  ...
		  <legend >审批详情</legend>
					<table class="table-form">
						<c:forEach items="${qmSupplier.examineList}" var="testData">
			<tr>
				...
				<tr>
					...
					<td class="tit" width="15%">是否需要客户查看：</td><td width="35%"  colspan="6">
							<form:radiobuttons path="field10" items="${fns:getDictList('is_leaf')}" itemLabel="label" itemValue="value"  onchange="orderType(this)" htmlEscape="false" class=""/>
		
					</td>
				</tr>	
					
				<tr id="field8Tr" style="display: none">
					
					<td class="tit" width="15%">用户客户：</td><td width="35%"  colspan="6">
						<sys:treeselect id="field8" name="field8" value="${qmSupplier.field8}" labelName="field9" labelValue="${qmSupplier.field9}"
					title="用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true"/>
				<span class="help-inline">选择客户 </span>
					</td>
				</tr>
			...
			<tr height="40px">
				...
				<tr>
					...
					<td colspan="6">
						<form:textarea path="field7" class="required" rows="5" maxlength="20" cssStyle="width:500px"/><span class="help-inline"><font color="red">*</font> </span>
					</td>
				</tr></table>
